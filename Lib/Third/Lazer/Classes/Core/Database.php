...
defined('LAZER_SECURE') or die('Permission denied!');
 ...
 abstract class Core_Database implements IteratorAggregate, Countable {
      ...
      * name变量改为_开头，要不数据库中不能含有name字段 By Javey
      */
     protected $_name;
...
//         $self->_data = Helpers\Data::table($self->_name)->get();
     ...
     {
         return Data::table($this->_name)->get();
     }
     ...
     {
         if (Validate::table($this->_name)->field($name) && Validate::table($this->_name)->type($name, $value))
         {
             $this->set->{$name} = $value;
         }
     }
...
//         $fields = Helpers\Validate::arrToLower($fields);
     ...
     {
         if (Validate::table($this->_name)->field($column))
         {
             $this->resetKeys = 0;
...
//             $local = (count($join) > 1) ? array_slice($join, -2, 1)[0] : $this->_name;
     ...
     {
         if (Validate::table($this->_name)->field($key))
         {
             $directions = array(
                 'ASC' => SORT_ASC,
                 'DESC' => SORT_DESC
     ...
     {
         uasort($this->data, array($this, 'uasortOrder'));
     }
      ...
      * @return bool|int
      */
     private function uasortOrder($a, $b) {
         ...
         foreach ($properties as $column => $direction)
         {
             ...
             $aProp = $this->collapse($a, $column);
             $bProp = $this->collapse($b, $column);
      ...
      * @return null
      */
     private function collapse($node, $props) {
         if (is_array($props))
         {
...
//         );
         ...
         $this->data = array_filter($this->data, array($this, 'filterOperator'));
     }

     private function filterOperator($row) {
         $operator = array(
             '=' => '==',
             '!=' => '!=',
             '>' => '>',
     ...
     public function asArray($key = null, $value = null)
     {
         if (!is_null($key))
         {
             Validate::table($this->_name)->field($key);
         }
         if (!is_null($value))
         {
             Validate::table($this->_name)->field($value);
         }
...
//         $fields = Validate::arrToLower($fields);
         ...
         Validate::table($this->_name)->fields($fields);
     ...
     {
         return Config::table($this->_name)->get();
     }
     ...
     {
         return Config::table($this->_name)->fields();
     }
     ...
     {
         return Config::table($this->_name)->schema();
     }
     ...
     {
         return Config::table($this->_name)->relations($tableName, true);
     }
     ...
     {
         return Config::table($this->_name)->lastId();
     }
...
//         $this->setFields();
     ...
     public function delete()
     {
         ...
         elseif (isset($this->pending['where']) && !empty($this->pending['where']))
         {
             $this->data = $data;
             $old = $this->data;
             call_user_func(array($this, 'wherePending'));
         ...
         return Data::table($this->_name)->put($this->data) ? true : false;
     }
