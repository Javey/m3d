 ...
 *
 * Last modified by luoqin @2012.07 
 *
...
class simple_html_dom_node {
    ...
    function makeup()
    {
        ...
        {
            ++$i;
            /* remove surfix from $key to write back the correct attr name, which is added in read_tag; 
                all of this are modified to prevend one attribute (with no value) not able to show twice, by lutaining @2012.08.23 */
...
class simple_html_dom {
    ...
    function load($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT) {
        ...
        //$this->remove_noise("'<!--[^\[].*?-->'is");
		//just only exclude interupt, not remove it!
		//and that will be removed by another place, like comments
...
//		$this->remove_noise("'(\{\\$\w)(.*?)(\})'s", true);
    ...
    function dbg() {
	    return nl2br(var_dump($this->noise)).'<p>===2 noise 2 doc====</p>'.nl2br(htmlspecialchars($this->doc));
    }
    ...
    protected function read_tag() {
        ...
        do
        {
            /* the following is commented to allow smarty tag like {/if} to show right after an attribute's value, with no space, by lutaining @2012.08.23 */
            ...
            if ($name!=='/' && $name!=='') {
                ...
                /* add surfix to $key, it is meant to prevend one attribute (with no value) not able to show twice, by lutaining @2012.08.23 */
